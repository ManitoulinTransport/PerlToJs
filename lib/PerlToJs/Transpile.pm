...
my %module_js = ();
my $base_command = 'perl ' . PerlToJs::Constants::BASE_PATH . '/Perlito/perlito5.pl -Cjs --noexpand_use -I' . PerlToJs::Constants::BASE_PATH . '/Perlito/src5/lib ';
sub getModuleJs {
	my ($module_path) = @_;
	unless (defined $module_js{$module_path}){
		$module_js{$module_path} = `$base_command$module_path` . '; ';
...
sub getModulesJs {
	...
	join("\n", map {
		...
		my $module_js = eval { getModuleJs($module_path) };
